...
describe('PriceOracleProxy', () => {
  ...
  beforeEach(async () => {
    ...
    cEth = await makeAToken({kind: "cether", comptrollerOpts: {kind: "v1-no-proxy"}, supportMarket: true});
    cUsdc = await makeAToken({comptroller: cEth.comptroller, supportMarket: true});
    cSai = await makeAToken({comptroller: cEth.comptroller, supportMarket: true});
    cDai = await makeAToken({comptroller: cEth.comptroller, supportMarket: true});
    cUsdt = await makeAToken({comptroller: cEth.comptroller, supportMarket: true});
    cOther = await makeAToken({comptroller: cEth.comptroller, supportMarket: true});
  ...
  describe("getUnderlyingPrice", () => {
    ...
    it("returns 0 for token without a price", async () => {
      let unlistedToken = await makeAToken({comptroller: cEth.comptroller});
