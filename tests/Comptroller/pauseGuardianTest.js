...
const { makeComptroller, makeAToken } = require('../Utils/Aquarius');
  ...
  describe('setting paused', () => {
    ...
    describe('succeeding', () => {
      ...
      globalMethods.forEach(async (method) => {
        it(`only pause guardian or admin can pause ${method}`, async () => {
          await expect(send(comptroller, `_set${method}Paused`, [true], {from: accounts[2]})).rejects.toRevert("revert only pause guardian, team pause guardian and admin can pause");
          await expect(send(comptroller, `_set${method}Paused`, [false], {from: accounts[2]})).rejects.toRevert("revert only pause guardian, team pause guardian and admin can pause");
        });
    ...
    describe('succeeding', () => {
      ...
      marketMethods.forEach(async (method) => {
        it(`only pause guardian or admin can pause ${method}`, async () => {
          await expect(send(comptroller, `_set${method}Paused`, [aToken._address, true], {from: accounts[2]})).rejects.toRevert("revert only pause guardian, team pause guardian and admin can pause");
          await expect(send(comptroller, `_set${method}Paused`, [aToken._address, false], {from: accounts[2]})).rejects.toRevert("revert only pause guardian, team pause guardian and admin can pause");
        });

        it(`PauseGuardian can pause of ${method}GuardianPaused`, async () => {
          result = await send(comptroller, `_set${method}Paused`, [aToken._address, true], {from: pauseGuardian});
          expect(result).toHaveLog(`ActionPaused`, {aToken: aToken._address, action: method, pauseState: true});

          let camelCase = method.charAt(0).toLowerCase() + method.substring(1);

          state = await call(comptroller, `${camelCase}GuardianPaused`, [aToken._address]);
          expect(state).toEqual(true);

          await expect(send(comptroller, `_set${method}Paused`, [aToken._address, false], {from: pauseGuardian})).rejects.toRevert("revert only admin can unpause");
          result = await send(comptroller, `_set${method}Paused`, [aToken._address, false]);

          expect(result).toHaveLog(`ActionPaused`, {aToken: aToken._address, action: method, pauseState: false});
        ...
        it(`pauses ${method}`, async() => {
          await send(comptroller, `_set${method}Paused`, [aToken._address, true], {from: pauseGuardian});
          switch (method) {
          case "Mint":
            expect(await call(comptroller, 'mintAllowed', [address(1), address(2), 1])).toHaveTrollError('MARKET_NOT_LISTED');
            await expect(send(comptroller, 'mintAllowed', [aToken._address, address(2), 1])).rejects.toRevert(`revert ${method.toLowerCase()} is paused`);
            break;

          case "Borrow":
            expect(await call(comptroller, 'borrowAllowed', [address(1), address(2), 1])).toHaveTrollError('MARKET_NOT_LISTED');
            await expect(send(comptroller, 'borrowAllowed', [aToken._address, address(2), 1])).rejects.toRevert(`revert ${method.toLowerCase()} is paused`);
            break;
