...
describe('ArsScenario', () => {
  let root, accounts;
  let ars;

  beforeEach(async () => {
    [root, ...accounts] = saddle.accounts;
    ars = await deploy('ArsScenario', [root]);
  });
    ...
    [
      ...
      it(`with ${checkpoints} checkpoints, has ${expectedReads} reads`, async () => {
        ...
        while (remaining > 0) {
          ...
          await ars.methods.generateCheckpoints(amt, offset).send({from: root, gas: 200000000});
          remaining -= amt;
          offset += amt;
        }

        let result = await ars.methods.getPriorVotes(root, 1).send();
