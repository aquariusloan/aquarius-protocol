...
describe.skip('ArsWheelFuzzTest', () => {
  // This whole test is fake, but we're testing to see if our equations match reality.

  // First, we're going to build a simple simulator of the Aquarius protocol
  ...
  let initialState = globals => {
    return {
      ...
      arsSupplySpeed: new bn(1),
      arsSupplyIndex: new bn(1),
      arsSupplyIndexSnapshots: {},
      arsSupplyIndexUpdatedBlock: globals.blockNumber,

      arsBorrowSpeed: new bn(1),
      arsBorrowIndex: new bn(1),
      arsBorrowIndexSnapshots: {},
      arsSupplyIndexUpdatedBlock: globals.blockNumber,

      arsAccruedWithCrank: {}, // naive method, accruing all accounts every block
      arsAccruedWithIndex: {}, // with indices

      activeBorrowBlocks: new bn(0), // # blocks with an active borrow, for which we expect to see ars distributed. just for fuzz testing.
  ...
  let trueUpArs = (globals, state) => {
    state = accrueInterest(globals, state);

    state = Object.keys(state.arsSupplyIndexSnapshots).reduce(
      (acc, account) => supplierFlywheelByIndex(globals, state, account),
      state
    );

    state = Object.keys(state.arsBorrowIndexSnapshots).reduce(
  ...
  // manual flywheel loops through every account and updates ars accrued mapping
  ...
  let flywheelByCrank = (
    ...
    for (let [account, balance] of Object.entries(balances)) {
      if (isPositive(totalSupply)) {
        arsAccruedWithCrank[account] = get(
          state.arsAccruedWithCrank[account]
        ).plus(
          deltaBlocks
            .times(arsSupplySpeed)
            .times(balance)
            .div(totalSupply)
        );
      }
    ...
    return {
      ...state,
      arsAccruedWithCrank: arsAccruedWithCrank,
    };
  };

  // real deal ars index flywheel™️
  let borrowerFlywheelByIndex = (globals, state, account) => {
    let {
      ...
      totalBorrows,
      borrowBalances,
      arsBorrowIndexUpdatedBlock,
      borrowIndex,
      borrowIndexSnapshots
    } = state;

    let deltaBlocks = globals.blockNumber.minus(arsBorrowIndexUpdatedBlock);
    if (isPositive(totalBorrows)) {
      let scaledTotalBorrows = totalBorrows.div(borrowIndex);
      arsBorrowIndex = arsBorrowIndex.plus(
        arsBorrowSpeed.times(deltaBlocks).div(scaledTotalBorrows)
      );
    }

    let indexSnapshot = arsBorrowIndexSnapshots[account];

    if (
      indexSnapshot !== undefined &&
      arsBorrowIndex.isGreaterThan(indexSnapshot) &&
      ...
      arsAccruedWithIndex[account] = get(arsAccruedWithIndex[account]).plus(
        borrowBalanceNew
          .div(borrowIndex)
          .times(arsBorrowIndex.minus(indexSnapshot))
      );
    }

    return {
      ...state,
      arsBorrowIndexUpdatedBlock: globals.blockNumber,
      arsBorrowIndex: arsBorrowIndex,
      arsBorrowIndexSnapshots: {
        ...state.arsBorrowIndexSnapshots,
        [account]: arsBorrowIndex
      }
    };
  };
  ...
  let supplierFlywheelByIndex = (globals, state, account) => {
    let {
      ...
      arsAccruedWithIndex,
      totalSupply,
      arsSupplyIndexUpdatedBlock
    } = state;

    let deltaBlocks = globals.blockNumber.minus(arsSupplyIndexUpdatedBlock);

    if (isPositive(totalSupply)) {
      arsSupplyIndex = arsSupplyIndex.plus(
        arsSupplySpeed.times(deltaBlocks).div(totalSupply)
      );
    }

    let indexSnapshot = arsSupplyIndexSnapshots[account];
    if (indexSnapshot !== undefined) {
      // if had prev snapshot,  accrue some Ars
      arsAccruedWithIndex[account] = get(arsAccruedWithIndex[account]).plus(
        balances[account].times(arsSupplyIndex.minus(indexSnapshot))
      );
    }
    ...
    return {
      ...state,
      arsSupplyIndexUpdatedBlock: globals.blockNumber,
      arsSupplyIndex: arsSupplyIndex,
      arsSupplyIndexSnapshots: {
        ...state.arsSupplyIndexSnapshots,
        [account]: arsSupplyIndex
      },
      arsAccruedWithIndex: arsAccruedWithIndex
    };
  };
  ...
  // assert amount distributed by the crank is expected, that it equals # blocks with a supply * ars speed
  let crankCorrectnessInvariant = (globals, state, events, invariantFn) => {
    let expected = state.activeSupplyBlocks
      .times(state.arsSupplySpeed)
      .plus(state.activeBorrowBlocks.times(state.arsBorrowSpeed));

    let actual = Object.values(state.arsAccruedWithCrank).reduce(
    ...
    invariantFn(
      ...
      expected,
      actual,
      `crank method distributed ars inaccurately`
    );
  };

  // assert ars distributed by index is the same as amount distributed by crank
  let indexCorrectnessInvariant = (globals, state, events, invariantFn) => {
    let expected = state.arsAccruedWithCrank;
    let actual = state.arsAccruedWithIndex;
    invariantFn(
      (expected, actual) => {
  ...
  let runEvents = (globals, initState, events) => {
    ...
    return trueUpArs(globals, state);
  };
