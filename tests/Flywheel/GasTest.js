...
describe.skip('Flywheel trace ops', () => {
  ...
  beforeEach(async () => {
    ...
    await send(comptroller, '_addArsMarkets', [[market].map(c => c._address)]);
  });
  ...
  it('update supply index SSTOREs', async () => {
    ...
    await send(comptroller, 'setArsSpeed', [market._address, etherExp(0.5)]);

    const tx = await send(comptroller, 'harnessUpdateArsSupplyIndex', [market._address]);
  ...
  it('update borrow index SSTOREs', async () => {
    ...
    const tx = await send(comptroller, 'harnessUpdateArsBorrowIndex', [market._address, etherExp(1.1)]);
