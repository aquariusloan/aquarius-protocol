...
contract ComptrollerG7 is ComptrollerV5Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
     ...
     * @param arsSpeed New ARS speed for market
     */
    function setArsSpeedInternal(AToken aToken, uint arsSpeed) internal {
        uint currentArsSpeed = arsSpeeds[address(aToken)];
        if (currentArsSpeed != 0) {
            // note that ARS speed could be set to 0 to hars liquidity rewards for a market
            ...
            if (arsSupplyState[address(aToken)].index == 0 && arsSupplyState[address(aToken)].block == 0) {
                arsSupplyState[address(aToken)] = ArsMarketState({
                    index: arsInitialIndex,
                    block: safe32(getBlockNumber(), "block number exceeds 32 bits")
                });
            }

            if (arsBorrowState[address(aToken)].index == 0 && arsBorrowState[address(aToken)].block == 0) {
                arsBorrowState[address(aToken)] = ArsMarketState({
                    index: arsInitialIndex,
                    block: safe32(getBlockNumber(), "block number exceeds 32 bits")
                });
            }
        ...
        if (currentArsSpeed != arsSpeed) {
            arsSpeeds[address(aToken)] = arsSpeed;
            emit ArsSpeedUpdated(aToken, arsSpeed);
        }
    }
     ...
     * @param arsSpeed New ARS speed for market
     */
    function _setArsSpeed(AToken aToken, uint arsSpeed) public {
        require(adminOrInitializing(), "only admin can set ars speed");
        setArsSpeedInternal(aToken, arsSpeed);
    }
