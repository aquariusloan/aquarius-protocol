...
contract Comptroller is ComptrollerV7Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
    ...
    function fixBadAccruals(address[] calldata affectedUsers, uint[] calldata amounts) external {
        ...
        for (uint i = 0; i < affectedUsers.length; ++i) {
            ...
            currentAccrual = arsAccrued[user];

            amountToSubtract = amounts[i];

            // The case where the user has claimed and received an incorrect amount of ARS.
            // The user has less currently accrued than the amount they incorrectly received.
            if (amountToSubtract > currentAccrual) {
                // Amount of ARS the user owes the protocol
                uint accountReceivable = amountToSubtract - currentAccrual; // Underflow safe since amountToSubtract > currentAccrual

                uint oldReceivable = arsReceivable[user];
                uint newReceivable = add_(oldReceivable, accountReceivable);

                // Accounting: record the ARS debt for the user
                arsReceivable[user] = newReceivable;

                emit ArsReceivableUpdated(user, oldReceivable, newReceivable);
            ...
            if (amountToSubtract > 0) {
                ...
                arsAccrued[user] = newAccrual = sub_(currentAccrual, amountToSubtract);

                emit ArsAccruedAdjusted(user, currentAccrual, newAccrual);
            }
        }
    ...
    function getArsAddress() public view returns (address) {
        return 0x4AC2618aceEaBe8CB5E248f5c3Fd5F0F503f6E3f;
    }
}
