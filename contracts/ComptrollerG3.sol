...
contract ComptrollerG3 is ComptrollerV3Storage, ComptrollerInterface, ComptrollerErrorReporter, Exponential {
    ...
    /// @notice Emitted when market arsed status is changed
    event MarketComped(AToken aToken, bool isComped);

    /// @notice Emitted when ARS rate is changed
    event NewArsRate(uint oldArsRate, uint newArsRate);

    /// @notice Emitted when a new ARS speed is calculated for a market
    event ArsSpeedUpdated(AToken indexed aToken, uint newSpeed);
    ...
    /// @notice The threshold above which the flywheel transfers ARS, in wei
    uint public constant arsClaimThreshold = 0.001e18;
    ...
    function mintAllowed(address aToken, address minter, uint mintAmount) external returns (uint) {
        ...
        distributeSupplierArs(aToken, minter, false);
    ...
    function redeemAllowed(address aToken, address redeemer, uint redeemTokens) external returns (uint) {
        ...
        // Keep the flywheel moving
        updateArsSupplyIndex(aToken);
        distributeSupplierArs(aToken, redeemer, false);
    ...
    function borrowAllowed(address aToken, address borrower, uint borrowAmount) external returns (uint) {
        ...
        distributeBorrowerArs(aToken, borrower, borrowIndex, false);
    ...
    function seizeAllowed(
        ...
        distributeSupplierArs(aTokenCollateral, borrower, false);
        distributeSupplierArs(aTokenCollateral, liquidator, false);
    ...
    function transferAllowed(address aToken, address src, address dst, uint transferTokens) external returns (uint) {
        ...
        // Keep the flywheel moving
        updateArsSupplyIndex(aToken);
        distributeSupplierArs(aToken, src, false);
        distributeSupplierArs(aToken, dst, false);
    ...
    function _become(Unitroller unitroller, uint arsRate_, address[] memory arsMarketsToAdd, address[] memory otherMarketsToAdd) public {
        require(msg.sender == unitroller.admin(), "only unitroller admin can change brains");
        require(unitroller._acceptImplementation() == 0, "change not authorized");

        ComptrollerG3(address(unitroller))._becomeG3(arsRate_, arsMarketsToAdd, otherMarketsToAdd);
    }

    function _becomeG3(uint arsRate_, address[] memory arsMarketsToAdd, address[] memory otherMarketsToAdd) public {
        require(msg.sender == comptrollerImplementation, "only brains can become itself");

        for (uint i = 0; i < arsMarketsToAdd.length; i++) {
            _addMarketInternal(address(arsMarketsToAdd[i]));
        }
        ...
        _setArsRate(arsRate_);
        _addArsMarkets(arsMarketsToAdd);
    }
    ...
    function refreshArsSpeeds() public {
        ...
        for (uint i = 0; i < allMarkets_.length; i++) {
            ...
            arsSpeeds[address(aToken)] = newSpeed;
            emit ArsSpeedUpdated(aToken, newSpeed);
        }
    }
    ...
    function updateArsSupplyIndex(address aToken) internal {
        ...
        uint supplySpeed = arsSpeeds[aToken];
        ...
        if (deltaBlocks > 0 && supplySpeed > 0) {
            ...
            arsSupplyState[aToken] = ArsMarketState({
    ...
    function updateArsBorrowIndex(address aToken, Exp memory marketBorrowIndex) internal {
        ...
        uint borrowSpeed = arsSpeeds[aToken];
        uint blockNumber = getBlockNumber();
        ...
        if (deltaBlocks > 0 && borrowSpeed > 0) {
            ...
            arsBorrowState[aToken] = ArsMarketState({
                index: safe224(index.mantissa, "new index exceeds 224 bits"),
                block: safe32(blockNumber, "block number exceeds 32 bits")
            });
        } else if (deltaBlocks > 0) {
     ...
     */
    function distributeSupplierArs(address aToken, address supplier, bool distributeAll) internal {
        ArsMarketState storage supplyState = arsSupplyState[aToken];
        Double memory supplyIndex = Double({mantissa: supplyState.index});
        Double memory supplierIndex = Double({mantissa: arsSupplierIndex[aToken][supplier]});
        arsSupplierIndex[aToken][supplier] = supplyIndex.mantissa;

        if (supplierIndex.mantissa == 0 && supplyIndex.mantissa > 0) {
            supplierIndex.mantissa = arsInitialIndex;
        }
        ...
        arsAccrued[supplier] = transferArs(supplier, supplierAccrued, distributeAll ? 0 : arsClaimThreshold);
        emit DistributedSupplierArs(AToken(aToken), supplier, supplierDelta, supplyIndex.mantissa);
    }
     ...
     */
    function distributeBorrowerArs(address aToken, address borrower, Exp memory marketBorrowIndex, bool distributeAll) internal {
        ArsMarketState storage borrowState = arsBorrowState[aToken];
        Double memory borrowIndex = Double({mantissa: borrowState.index});
        Double memory borrowerIndex = Double({mantissa: arsBorrowerIndex[aToken][borrower]});
        arsBorrowerIndex[aToken][borrower] = borrowIndex.mantissa;
        ...
        if (borrowerIndex.mantissa > 0) {
            ...
            arsAccrued[borrower] = transferArs(borrower, borrowerAccrued, distributeAll ? 0 : arsClaimThreshold);
            emit DistributedBorrowerArs(AToken(aToken), borrower, borrowerDelta, borrowIndex.mantissa);
        }
    }

    /**
     * @notice Transfer ARS to the user, if they are above the threshold
     ...
     * @param userAccrued The amount of ARS to (possibly) transfer
     * @return The amount of ARS which was NOT transferred to the user
     */
    function transferArs(address user, uint userAccrued, uint threshold) internal returns (uint) {
        if (userAccrued >= threshold && userAccrued > 0) {
            Ars ars = Ars(getArsAddress());
            uint arsRemaining = ars.balanceOf(address(this));
            if (userAccrued <= arsRemaining) {
                ars.transfer(user, userAccrued);
                return 0;
            }
        }
        return userAccrued;
        ...
        for (uint i = 0; i < aTokens.length; i++) {
            ...
            if (borrowers == true) {
                ...
                for (uint j = 0; j < holders.length; j++) {
                    distributeBorrowerArs(address(aToken), holders[j], borrowIndex, true);
                }
            }
            if (suppliers == true) {
                updateArsSupplyIndex(address(aToken));
                for (uint j = 0; j < holders.length; j++) {
                    distributeSupplierArs(address(aToken), holders[j], true);
                }
            }
        }
    }
     ...
     */
    function _setArsRate(uint arsRate_) public {
        require(adminOrInitializing(), "only admin can change ars rate");

        uint oldRate = arsRate;
        arsRate = arsRate_;
        emit NewArsRate(oldRate, arsRate_);

        refreshArsSpeeds();
    }

    /**
     * @notice Add markets to arsMarkets, allowing them to earn ARS in the flywheel
     * @param aTokens The addresses of the markets to add
     */
    function _addArsMarkets(address[] memory aTokens) public {
        require(adminOrInitializing(), "only admin can add ars market");

        for (uint i = 0; i < aTokens.length; i++) {
            _addArsMarketInternal(aTokens[i]);
        }

        refreshArsSpeeds();
    }

    function _addArsMarketInternal(address aToken) internal {
        Market storage market = markets[aToken];
        require(market.isListed == true, "ars market is not listed");
        require(market.isComped == false, "ars market already added");

        market.isComped = true;
        emit MarketComped(AToken(aToken), true);

        if (arsSupplyState[aToken].index == 0 && arsSupplyState[aToken].block == 0) {
            arsSupplyState[aToken] = ArsMarketState({
                index: arsInitialIndex,
                block: safe32(getBlockNumber(), "block number exceeds 32 bits")
            });
        }

        if (arsBorrowState[aToken].index == 0 && arsBorrowState[aToken].block == 0) {
            arsBorrowState[aToken] = ArsMarketState({
                index: arsInitialIndex,
                block: safe32(getBlockNumber(), "block number exceeds 32 bits")
            });
    ...
    /**
     * @notice Remove a market from arsMarkets, preventing it from earning ARS in the flywheel
     * @param aToken The address of the market to drop
     */
    function _dropArsMarket(address aToken) public {
        require(msg.sender == admin, "only admin can drop ars market");

        Market storage market = markets[aToken];
        require(market.isComped == true, "market is not a ars market");
