...
spec/certora/Ars/search.cvl:
	$(CERTORA_RUN) \
	spec/certora/contracts/ArsCertora.sol \
	--settings -b=4,-graphDrawLimit=0,-assumeUnwindCond,-depth=100 \
	--solc_args "'--evm-version istanbul'" \
	--verify \
	 ArsCertora:$@

spec/certora/Ars/transfer.cvl:
	$(CERTORA_RUN) \
	spec/certora/contracts/ArsCertora.sol \
...
spec/certora/Governor/%.cvl:
	$(CERTORA_RUN) \
	 ...
	 GovernorAlphaCertora:ars=ArsCertora \
	--verify \
...
spec/certora/AErc20/%.cvl:
	...
	--settings -cache=certora-run-aerc20-immutable
...
spec/certora/AErc20Delegator/%.cvl:
	...
	--settings -cache=certora-run-aerc20-delegator
