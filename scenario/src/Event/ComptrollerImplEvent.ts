...
async function becomeG3(
  ...
  comptrollerImpl: ComptrollerImpl,
  unitroller: Unitroller,
  arsRate: encodedNumber,
  arsMarkets: string[],
  otherMarkets: string[]
): Promise<World> {
  let invokation = await invoke(
    world,
    comptrollerImpl.methods._become(unitroller._address, arsRate, arsMarkets, otherMarkets),
    from,
    ComptrollerErrorReporter
  );
...
export function comptrollerImplCommands() {
  return [
    ...
    new Command<{
      ...
      comptrollerImpl: ComptrollerImpl;
      arsRate: NumberV;
      arsMarkets: ArrayV<AddressV>;
      otherMarkets: ArrayV<AddressV>;
    }>(
      `
        #### BecomeG3

        * "ComptrollerImpl <Impl> BecomeG3 <Rate> <ArsMarkets> <OtherMarkets>" - Become the comptroller, if possible.
          * E.g. "ComptrollerImpl MyImpl BecomeG3 0.1e18 [aDAI, cETH, aUSDC]
      `,
      'BecomeG3',
      [
        new Arg('unitroller', getUnitroller, { implicit: true }),
        new Arg('comptrollerImpl', getComptrollerImpl),
        new Arg('arsRate', getNumberV, { default: new NumberV(1e18) }),
        new Arg('arsMarkets', getArrayV(getAddressV),  {default: new ArrayV([]) }),
        new Arg('otherMarkets', getArrayV(getAddressV), { default: new ArrayV([]) })
      ],
      (world, from, { unitroller, comptrollerImpl, arsRate, arsMarkets, otherMarkets }) => {
        return becomeG3(world, from, comptrollerImpl, unitroller, arsRate.encode(), arsMarkets.val.map(a => a.val), otherMarkets.val.map(a => a.val))
      },
      { namePos: 1 }
    ),
    ...
    new Command<{
      ...
      `
        ...
        * "ComptrollerImpl <Impl> Become <Rate> <ArsMarkets> <OtherMarkets>" - Become the comptroller, if possible.
          * E.g. "ComptrollerImpl MyImpl Become 0.1e18 [aDAI, cETH, aUSDC]
      `,
      'Become',
      [
        new Arg('unitroller', getUnitroller, { implicit: true }),
