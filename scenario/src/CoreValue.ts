...
import { getArsValue, arsFetchers } from './Value/ArsValue';
...
const fetchers = [
    ...
    async (world, { addr, slot, key, nestedKey, valType }) => {
      ...
      switch (valType.val) {
        case 'marketStruct':
          ...
          let isArsKey = '0x' + toBN(newKey).add(toBN(3)).toString(16);
          let isArsStr = await world.web3.eth.getStorageAt(addr.val, isArsKey);

          return new ListV([
            new BoolV(isListed),
            new ExpNumberV(collateralFactor.toString(), 1e18),
            new BoolV(areEqual(userInMarket, 1)),
            new BoolV(areEqual(isArsStr, 1))
          ]);
        default:
          return new NothingV();
      }
  ...
  new Fetcher<{ res: Value }, Value>(
    `
      ...
      * "Ars ...arsArgs" - Returns Ars value
    `,
    'Ars',
    [new Arg('res', getArsValue, { variadic: true })],
    async (world, { res }) => res,
    { subExpressions: arsFetchers() }
  ),
  new Fetcher<{ res: Value }, Value>(
    `
      #### Governor
...
let contractFetchers = [
  ...
  { contract: "AquariusLens", implicit: false },
