...
async function getArsMarkets(world: World, comptroller: Comptroller): Promise<ListV> {
  let mkts = await comptroller.methods.getArsMarkets().call();
...
async function checkIsComped(world: World, comptroller: Comptroller, aToken: AToken): Promise<BoolV> {
  let {0: isListed, 1: _collateralFactorMantissa, 2: isComped} = await comptroller.methods.markets(aToken._address).call();
...
export function comptrollerFetchers() {
  return [
    ...
    new Fetcher<{comptroller: Comptroller}, ListV>(`
      ...
      * "GetArsMarkets" - Returns an array of the currently enabled Ars markets. To use the auto-gen array getter arsMarkets(uint), use ArsMarkets
      * E.g. "Comptroller GetArsMarkets"
      `,
      "GetArsMarkets",
      [new Arg("comptroller", getComptroller, {implicit: true})],
      async(world, {comptroller}) => await getArsMarkets(world, comptroller)
     ),

    new Fetcher<{comptroller: Comptroller}, NumberV>(`
      #### ArsRate

      * "ArsRate" - Returns the current ars rate.
      * E.g. "Comptroller ArsRate"
      `,
      "ArsRate",
      [new Arg("comptroller", getComptroller, {implicit: true})],
      async(world, {comptroller}) => new NumberV(await comptroller.methods.arsRate().call())
    ),
    ...
    new Fetcher<{comptroller: Comptroller, signature: StringV, callArgs: StringV[]}, NumberV>(`
        ...
        * "CallNum signature:<String> ...callArgs<CoreValue>" - Simple direct call method
          * E.g. "Comptroller CallNum \"arsSpeeds(address)\" (Address Coburn)"
      `,
      "CallNum",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
    ...
    new Fetcher<{comptroller: Comptroller, AToken: AToken, key: StringV}, NumberV>(`
        #### ArsSupplyState(address)

        * "Comptroller ArsBorrowState aZRX "index"
      `,
      "ArsSupplyState",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
      ...
      async (world, {comptroller, AToken, key}) => {
        const result = await comptroller.methods.arsSupplyState(AToken._address).call();
        return new NumberV(result[key.val]);
      }
    ),
    new Fetcher<{comptroller: Comptroller, AToken: AToken, key: StringV}, NumberV>(`
        #### ArsBorrowState(address)
      ...
      async (world, {comptroller, AToken, key}) => {
        const result = await comptroller.methods.arsBorrowState(AToken._address).call();
        return new NumberV(result[key.val]);
      }
    ),
    new Fetcher<{comptroller: Comptroller, account: AddressV, key: StringV}, NumberV>(`
      ...
      async (world, {comptroller,account}) => {
        const result = await comptroller.methods.arsAccrued(account.val).call();
        return new NumberV(result);
      }
    ),
    new Fetcher<{comptroller: Comptroller, account: AddressV, key: StringV}, NumberV>(`
        #### ArsReceivable(address)

        * "Comptroller ArsReceivable Coburn
      `,
      "ArsReceivable",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
      ...
      async (world, {comptroller,account}) => {
        const result = await comptroller.methods.arsReceivable(account.val).call();
        return new NumberV(result);
      }
    ),
    new Fetcher<{comptroller: Comptroller, AToken: AToken, account: AddressV}, NumberV>(`
        ...
        * "Comptroller ArsSupplierIndex aZRX Coburn
      `,
      "ArsSupplierIndex",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
      ...
      async (world, {comptroller, AToken, account}) => {
        return new NumberV(await comptroller.methods.arsSupplierIndex(AToken._address, account.val).call());
      }
    ),
    new Fetcher<{comptroller: Comptroller, AToken: AToken, account: AddressV}, NumberV>(`
        #### ArsBorrowerIndex

        * "Comptroller ArsBorrowerIndex aZRX Coburn
      `,
      "ArsBorrowerIndex",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
      ...
      async (world, {comptroller, AToken, account}) => {
        return new NumberV(await comptroller.methods.arsBorrowerIndex(AToken._address, account.val).call());
      }
    ),
    new Fetcher<{comptroller: Comptroller, AToken: AToken}, NumberV>(`
        #### ArsSpeed

        * "Comptroller ArsSpeed aZRX
      `,
      "ArsSpeed",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
      ...
      async (world, {comptroller, AToken}) => {
        return new NumberV(await comptroller.methods.arsSpeeds(AToken._address).call());
      }
    ),
    new Fetcher<{comptroller: Comptroller, AToken: AToken}, NumberV>(`
        #### ArsSupplySpeed

        * "Comptroller ArsSupplySpeed aZRX
      `,
      "ArsSupplySpeed",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
      ...
      async (world, {comptroller, AToken}) => {
        return new NumberV(await comptroller.methods.arsSupplySpeeds(AToken._address).call());
      }
    ),
    new Fetcher<{comptroller: Comptroller, AToken: AToken}, NumberV>(`
        #### ArsBorrowSpeed

        * "Comptroller ArsBorrowSpeed aZRX
      `,
      "ArsBorrowSpeed",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
      ...
      async (world, {comptroller, AToken}) => {
        return new NumberV(await comptroller.methods.arsBorrowSpeeds(AToken._address).call());
      }
    ),
    new Fetcher<{comptroller: Comptroller}, AddressV>(`
        #### BorrowCapGuardian
