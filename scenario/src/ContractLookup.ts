...
export async function getArs(
  world: World,
  arsArg: Event
): Promise<Ars> {
  return getWorldContract(world, [['ARS', 'address']]);
}

export async function getArsData(
  world: World,
  arsArg: string
): Promise<[Ars, string, Map<string, string>]> {
  let contract = await getArs(world, <Event>(<any>arsArg));
  let data = getContractData(world, [['Ars', arsArg]]);

  return [contract, arsArg, <Map<string, string>>(<any>data)];
}
