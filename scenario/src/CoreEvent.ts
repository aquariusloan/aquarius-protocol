...
import { arsCommands, processArsEvent } from './Event/ArsEvent';
...
import { AquariusLens } from './Contract/AquariusLens';
...
export const commands: (View<any> | ((world: World) => Promise<View<any>>))[] = [
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "Trx ...trxEvent" - Handles event to set details of next transaction
        * E.g. "Trx Value 1.0e18 (AToken cEth Mint 1.0e18)"
    `,
    'Trx',
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * E.g. "PriceOracleProxy Deploy (Unitroller Address) (PriceOracle Address) (AToken cETH Address)"
    `,
    'PriceOracleProxy',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => {
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * E.g. "Maximillion Deploy (AToken cETH Address)"
    `,
    'Maximillion',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => {
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "Ars ...event" - Runs given ars event
      * E.g. "Ars Deploy"
    `,
    'Ars',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => {
      return processArsEvent(world, event.val, from);
    },
    { subExpressions: arsCommands() }
  ),
  ...
  buildContractEvent<AquariusLens>("AquariusLens", false),
