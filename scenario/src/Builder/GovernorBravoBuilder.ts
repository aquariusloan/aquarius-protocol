...
export async function buildGovernor(
  ...
  const fetchers = [
    new Fetcher<
      { name: StringV, timelock: AddressV, ars: AddressV, admin: AddressV, implementation: AddressV, votingPeriod: NumberV, votingDelay: NumberV, proposalThreshold: NumberV},
      GovernorBravoData
    >(
      `
      #### GovernorBravoDelegator

      * "GovernorBravo Deploy BravoDelegator name:<String> timelock:<Address> ars:<Address> admin:<Address> implementation<address> votingPeriod:<Number> votingDelay:<Number>" - Deploys Aquarius Governor Bravo with a given parameters
        * E.g. "GovernorBravo Deploy BravoDelegator GovernorBravo (Address Timelock) (Address Ars) Admin (Address impl) 129600 1"
    `,
      "BravoDelegator",
      [
        new Arg("name", getStringV),
        new Arg("timelock", getAddressV),
        new Arg("ars", getAddressV),
      ...
      async (world, { name, timelock, ars, admin, implementation, votingPeriod, votingDelay, proposalThreshold }) => {
        return {
          invokation: await GovernorBravoDelegator.deploy<GovernorBravo>(
            world,
            from,
            [timelock.val, ars.val, admin.val, implementation.val, votingPeriod.encode(), votingDelay.encode(), proposalThreshold.encode()]
          ),
          name: name.val,
    ...
    new Fetcher<
      { name: StringV, timelock: AddressV, ars: AddressV, admin: AddressV, votingPeriod: NumberV, votingDelay: NumberV, proposalThreshold: NumberV },
      GovernorBravoData
    >(
      `
      #### GovernorBravoImmutable

      * "GovernorBravoImmut Deploy BravoImmutable name:<String> timelock:<Address> ars:<Address> admin:<Address> votingPeriod:<Number> votingDelay:<Number>" - Deploys Aquarius Governor Bravo Immutable with a given parameters
        * E.g. "GovernorBravo Deploy BravoImmutable GovernorBravo (Address Timelock) (Address Ars) Admin 129600 1"
    `,
      "BravoImmutable",
      [
        new Arg("name", getStringV),
      ...
      async (world, { name, timelock, ars, admin, votingPeriod, votingDelay, proposalThreshold }) => {
        return {
          invokation: await GovernorBravoImmutable.deploy<GovernorBravo>(
            world,
            from,
            [timelock.val, ars.val, admin.val, votingPeriod.encode(), votingDelay.encode(), proposalThreshold.encode()]
          ),
          name: name.val,
    ...
    >(
      ...
      * "Governor Deploy BravoDelegate name:<String>" - Deploys Aquarius Governor Bravo Delegate
    ...
    >(
      ...
      * "Governor Deploy BravoDelegateHarness name:<String>" - Deploys Aquarius Governor Bravo Delegate Harness
