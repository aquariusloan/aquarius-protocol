...
export async function buildGovernor(
  ...
  const fetchers = [
    new Fetcher<
      { name: StringV, timelock: AddressV, ars: AddressV, guardian: AddressV },
      GovernorData
    >(
      `
      #### GovernorAlpha

      * "Governor Deploy Alpha name:<String> timelock:<Address> ars:<Address> guardian:<Address>" - Deploys Aquarius Governor Alpha
        * E.g. "Governor Deploy Alpha GovernorAlpha (Address Timelock) (Address Ars) Guardian"
    `,
      "Alpha",
      [
      ...
      async (world, { name, timelock, ars, guardian }) => {
        return {
          invokation: await GovernorAlphaContract.deploy<Governor>(
            world,
            from,
            [timelock.val, ars.val, guardian.val]
          ),
          name: name.val,
    ...
    >(
      ...
      * "Governor Deploy AlphaHarness name:<String> timelock:<Address> ars:<Address> guardian:<Address>" - Deploys Aquarius Governor Alpha with a mocked voting period
        * E.g. "Governor Deploy AlphaHarness GovernorAlphaHarness (Address Timelock) (Address Ars) Guardian"
    `,
      "AlphaHarness",
      [
        new Arg("name", getStringV),
